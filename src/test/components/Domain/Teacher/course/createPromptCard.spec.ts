...
describe('CreatePromptCardコンポーネントのテスト', () => {
  ...
  describe('バリデーション', () => {
    ...
    it('全ての入力フィールドに有効な値が入力されている場合、エラーメッセージが表示されず、プロンプトを作成できる', async () => {
      ...
      expect(prompt.prompts.value[0].prompt_questions[0].content).toBe('今日学んだ内容について、自分が理解できた点とまだ理解できていない点は何ですか？')
      expect(prompt.prompts.value[0].prompt_questions[1].content).toBe('今日の授業で扱った数学の概念や公式を、日常生活や他の科目と関連付けることができますか？具体的な例を挙げて説明してください。')
    })
  })
})
